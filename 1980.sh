...
NEON_CYAN='\033[38;2;0;243;255m'
NEON_PINK='\033[38;2;255;0;255m'
NEON_GREEN='\033[38;2;57;255;20m'
NEON_AMBER='\033[38;2;255;191;0m'
RESET='\033[0m'
...
banner(){
  ...
  echo -e "${NEON_PINK}          [ ✓ ] KARMA-LOAD 2025 – FUTURISTIC STRESS TEST [ ✓ ]${RESET}"
  echo -e "${NEON_CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
  echo -e "${NEON_GREEN}    Web Dashboard : http://localhost:8888  (live stats)${RESET}"
  echo
}
...
menu(){
  while :;do
    banner
    echo -e "${NEON_GREEN}[1]${RESET} HTTP FLOOD"
    echo -e "${NEON_GREEN}[2]${RESET} SLOWLORIS"
    echo -e "${NEON_GREEN}[3]${RESET} KARMA HTTP FLOOD"
    echo -e "${NEON_GREEN}[4]${RESET} KARMA SLOW READ"
    echo -e "${NEON_GREEN}[0]${RESET} Exit"
    echo
    echo -n -e "${NEON_AMBER}[ ? ] Choose : ${RESET}"
    read -r p
    case $p in
      1)launch "HTTP FLOOD";;
      2)launch "SLOWLORIS";;
      ...
      0)exit 0;;
      *)echo -e "${NEON_PINK}[ ! ] Invalid${RESET}";sleep 1;;
    esac
  done
}
...
launch(){
  METHOD=$1
  echo -n -e "${NEON_CYAN}[ ✓ ] Target URL : ${RESET}";read -r TARGET
  validate_url "$TARGET" || { echo -e "${NEON_PINK}[ ! ] Bad URL${RESET}";sleep 1;return; }
  echo -n -e "${NEON_CYAN}[ ✓ ] Threads (1-1000) : ${RESET}";read -r THREADS
  ((THREADS>=1&&THREADS<=1000)) || { echo -e "${NEON_PINK}[ ! ] 1-1000 only${RESET}";sleep 1;return; }
  echo -n -e "${NEON_CYAN}[ ✓ ] Duration (s) : ${RESET}";read -r DUR
  ((DUR>=1)) || { echo -e "${NEON_PINK}[ ! ] Min 1s${RESET}";sleep 1;return; }
  build_python_engine
  echo -e "${NEON_GREEN}[ ✓ ] Starting $METHOD ...${RESET}"
  python3 "$TMP_PY" "$TARGET" "$METHOD" "$THREADS" "$DUR"
  echo;echo -n -e "${NEON_AMBER}[ ✓ ] Press ENTER to back${RESET}";read -r
}
